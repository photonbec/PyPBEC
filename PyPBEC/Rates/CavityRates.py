...
class CavityRates():
	...
	def __init__(self, rawDataPath='R6G_data', model='exp_data'):
		...
		elif self.model == 'exp_data_adj':
			self.get_normalized_absorption_spectrum()
	...
	#Previous simulations have used magic numbers to fit the functional form of the absorption (and equivalently emission) cross sections. It comes from Henry. I don't know where he got it from. Could be worth asking.
	...
	def get_normalized_absorption_spectrum(self):
		min_wavelength = 480
		max_wavelength = 650
		absorption_spectrum_datafile = Path(self.rawDataPath) / 'absorption_cross_sections_R6G_in_EthyleneGlycol_corrected.csv'
		...
		self.absorption_spectrum = np.squeeze(np.array([[absorption_data['wavelength (nm)'].values], [absorption_data_normalized]], dtype=float))
		self.interpolated_absorption_spectrum = interp1d(self.absorption_spectrum[0,:], self.absorption_spectrum[1,:], kind='cubic')
	...
	def compute_rates(self, lambdas, rho=2.3, n=1.43, peak_Xsectn = 2.45e-20, temperature=300, Gamma_down=None, Gamma_0=None, degeneracy=None):
		...
		if self.model == 'exp_data': # or self.model == 'magic_numbers':
			...
			absorption_Xsctns = [self.normalised_absorption(a_l)*self.peak_Xsectn for a_l in lambdas]
			absorption_rates = array(absorption_Xsctns)*n_mol_per_vol*sc.c/self.solvent_n
			emission = [self.normalised_absorption(a_l*self.lamZPL/(2.0*a_l-self.lamZPL))*self.peak_Xsectn for a_l in lambdas]
			emission_rates = array(emission)*n_mol_per_vol*sc.c/self.solvent_n
			#ks_factors = array([exp(sc.h*sc.c*(1/self.lamZPL - 1/lam)/(sc.Boltzmann*self.temperature)) for lam in lambdas])
			#emission_rates = absorption_rates*ks_factors
			return absorption_rates, emission_rates
			...
			self.temperature = temperature
			n_mol_per_vol= self.rho*sc.Avogadro
			self.peak_Xsectn = peak_Xsectn*n_mol_per_vol*sc.c/self.solvent_n
			wpzl = 2*pi*sc.c/self.lamZPL/1e12

			def freq(wl):
				return 2*pi*sc.c/wl/1e12
			def single_exp_func(det):
				f_p = 2*pi*sc.c/(wpzl+det)*1e-3
				f_m = 2*pi*sc.c/(wpzl-det)*1e-3
				return (0.5*self.interpolated_absorption_spectrum(f_p)) + (0.5*self.interpolated_emission_spectrum(f_m))
			def Err(det):
				return Erf(det*1e12)
			def single_adjust_func(det):
				return ((1+Err(det))/2.0*single_exp_func(det)) + ((1-Err(det))/2.0*single_exp_func(-1.0*det)*exp(sc.h/(2*pi*sc.k*self.temperature)*det*1e12)) 
			
			emission_rates = array([single_adjust_func(-1.0*freq(a_l)+wpzl) for a_l in lambdas])*self.peak_Xsectn
			absorption_rates = array([single_adjust_func(freq(a_l)-wpzl) for a_l in lambdas])*self.peak_Xsectn
			return absorption_rates, emission_rates
	...
	def KS_factor(self, lambdas, temperature=300):
			self.temperature = temperature
			ks_factors = array([exp(sc.h*sc.c*(1/self.lamZPL - 1/lam)/(sc.Boltzmann*self.temperature)) for lam in lambdas])
			return ks_factors
